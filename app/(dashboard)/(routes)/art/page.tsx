...
const ArtPage = () => {
  ...
  // Use an array of strings for images, since the backend returns string URLs
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      // data.data is an array of string URLs
      setImages(data.data);
  ...
  return (
    <div className="h-full p-4 space-y-6">
      <Heading
        title="AI Art Studio"
        description={
          isPro ? "Transform your imagination into stunning artwork." : `${apiLimitCount} / 5 Free Generations`
        }
        icon={ImageIcon}
                ...
                {images.map((url, index) => (
                  <Card key={index} className="overflow-hidden rounded-xl shadow-lg hover:shadow-xl transition-shadow">
                    <div className="relative aspect-square">
                      <Image alt="Generated art" src={url} fill className="object-cover" />
                    </div>
