  ...
  return (
    <main className='flex-1 p-6 md:p-8 max-w-7xl mx-auto'>
              ...
              {proModal.selectedTool && (
                <div className='flex items-center justify-center flex-col space-y-4'>
                  <div
                    className={cn(
                      "p-3 rounded-xl",
                      proModal.selectedTool.bgColor
                    )}>
                    <proModal.selectedTool.icon
                      className={cn("w-12 h-12", proModal.selectedTool.color)}
                    />
                  </div>
                  <div className='text-xl font-semibold'>
                    {proModal.selectedTool.label}
                  </div>
                  <p className='text-sm text-muted-foreground'>
                    {proModal.selectedTool.description}
                  </p>
                </div>
              )}
      ...
      <div className='space-y-8'>
        {Object.entries(groupBy(filteredTools, "featureType")).map(
          ([featureType, tools]) => (
          ...
          <CardContent>
            <div className='flex items-end justify-between h-[60px]'>
              {usageStats.dailyUsage.map((usage, index) => (
                <div
                  key={index}
                  className='w-[8%] bg-primary/10 rounded-sm'
                  style={{
                    height: `${(usage / Math.max(...usageStats.dailyUsage)) * 100}%`
                  }}
                />
              ))}
            </div>
